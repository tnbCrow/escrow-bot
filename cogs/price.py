...
class price(commands.Cog):
    ...
    async def rate(self, ctx):
        ...
        comma_seperated_supply = "{:,}".format(circulating_supply)
        ...
        comma_seperated_cap = "{:,}".format(market_cap)

        # create an embed and show it to users
        embed = discord.Embed(color=0xe81111)
        embed.add_field(name="Last Trade Rate", value=f"{last_rate} USDC", inline=False)
        embed.add_field(name="Circulating Supply", value=comma_seperated_supply, inline=False)
        embed.add_field(name="Market Cap", value=f"{comma_seperated_cap} USDC", inline=False)
